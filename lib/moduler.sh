...
App:Import()
{
	...
	if (cat "$file_or_src" &> /dev/null); then
		for __depencies__ in "$(cat ${file_or_src})"; do
			builtin source <(curl --max-time 4 --connect-timeout 3 --parallel --parallel-max 15 -sL "${__depencies__}" --insecure --compressed) || { echo "[*] Error Include"; exit 33; }
		done
	else {
			__depencies__="$file_or_src"
			builtin source <(curl --max-time 4 --connect-timeout 3 --parallel --parallel-max 15 -sL "${__depencies__}" --insecure --compressed) || { echo "[*] Error Include"; exit 33; }
	}; fi
...
Std::Sys()
{
	var:name()
	{
		...
		printf -v "${name_variabel}" "%b" "$(echo ${*:3})"
